...
void loop()
{
  ...
  display.clearDisplay();
  display.setCursor(0, 0);
  reading = analogRead(ANALOG_IN); //read analog value from pin PA3
  angle = (reading / 120);         //it divides ADC the value according to max angle 170 deg
  servo.write(angle);              //it puts angle value at servo

  display.println((analogRead(ANALOG_IN)));
  display.display();
  /*
